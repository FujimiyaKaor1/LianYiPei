"""
使用 Faker(zh_CN) 补全 enterprises 空字段，并按经营范围映射行业生成 products。

不清空任何表，保留库内真实企业数据。仅处理 enterprises.id > 0 的记录。

重复执行：默认仅对「当前无产品」的企业插入产品；使用 --force 可无视该限制（会追加 2～3 条）。
"""
...
def _manufacturing_description(product_name: str, industry_key: str, fake: Faker) -> str:
    """50～100 字的制造业产品说明（中文）。"""
    tail = random.choice(
        [
            "关键尺寸经三坐标与抽检控制，批次可追溯。",
            "满足常见耐温、防护与接地安全基线，可按项目验收。",
            "可与上位机或 MES 对接，支持参数下发与工单追溯。",
            "采用行业成熟工艺路线，交付含基本技术资料。",
        ]
    )
    mid = fake.sentence(nb_words=8).replace("\n", "").strip()
    if not mid.endswith("。"):
        mid += "。"
    text = (
        f"{product_name}面向工业现场与规模化制造场景，适用于{industry_key[:16]}相关需求。"
        f"{mid}{tail}"
    )
    text = text.replace("\n", "").replace(" ", "")
    if len(text) > 100:
...
def _embellish_product_name(base: str) -> str:
    """随机加专业前缀，贴近「高精密模具、工业自动化控制箱」等风格。"""
    prefixes = ("高精密", "大功率", "工业级", "智能型", "重载型", "模块化", "")
    p = random.choice(prefixes)
    b = base.strip()
    if not p or b.startswith(p):
        return b[:100]
    return f"{p}{b}"[:100]
...
def enrich_enterprises(fake: Faker, verbose: bool) -> int:
    enterprises = (
        Enterprise.query.filter(Enterprise.id > 0).order_by(Enterprise.id).all()
    )
    n = len(enterprises)
    ...
    for i, e in enumerate(enterprises, 1):
        ...
        if _empty_num(e.credit_score):
            e.credit_score = round(random.uniform(80, 95), 1)
            changed = True
...
def seed_products(fake: Faker, force: bool, verbose: bool) -> int:
    ...
    for i, e in enumerate(enterprises, 1):
        ...
        if len(pool) >= want:
            bases = random.sample(pool, k=want)
        else:
            bases = random.choices(pool, k=want)
        if verbose:
            print(f"  [{i}/{n}] id={e.id} 行业={industry_key} 产品数={want}")
        for base in bases:
            name = _embellish_product_name(base)
            desc = _manufacturing_description(name, industry_key, fake)
            p = Product(
                name=name[:100],
                description=desc,
    ...
    db.session.commit()
    print(
        f"[第三部分] 完成，新建 {created} 条产品；跳过 {skipped} 家企业（无 --force 且已有产品；每家 2~3 条）"
    )
    return created
...
def main() -> None:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="即使该企业已有产品仍追加 2~3 条（默认仅对无产品企业插入）",
    )
