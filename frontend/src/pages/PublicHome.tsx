...
import {
  ...
  Globe2, Leaf, Loader2, MapPin, Network, PackageSearch, Search,
...
const PUBLIC_SERVICES = [
  { key: 'search', eyebrow: '01 / 公开能力', title: '公开找厂', summary: '按行业、地区和能力筛选已收录企业，先看依据再联系。', action: '进入找厂', icon: Factory },
  { key: 'ai', eyebrow: '02 / 智能协同', title: 'AI 找工厂', summary: '用自然语言描述采购需求，让链小易整理匹配条件和下一步动作。', action: '让 AI 帮我找', icon: Sparkles },
  { key: 'collaboration', eyebrow: '03 / 业务协同', title: '供需协同', summary: '从询价、报价到订单履约，沉淀可追溯的协同记录。', action: '进入协同', icon: Network },
  { key: 'enterprise', eyebrow: '04 / 企业成长', title: '企业入驻', summary: '完善企业、产品、产能和资质档案，让真实能力被看见。', action: '申请入驻', icon: Building2 },
] as const;
...
export default function PublicHome() {
  ...
  const openLogin = (_intent = '进入链易配工作台') => requestLogin();
  ...
  const openService = (key: (typeof PUBLIC_SERVICES)[number]['key']) => {
    if (key === 'search') navigate('/search');
    else if (key === 'ai') serviceInputRef.current?.focus();
    else openLogin(key === 'enterprise' ? '企业入驻' : '进入供需协同');
  };
  const submitServiceQuery = (event: FormEvent) => {
    event.preventDefault();
    const query = serviceQuery.trim();
    if (query) navigate(`/aia?q=${encodeURIComponent(query)}`);
  };
  ...
  return <div className="public-site min-h-screen bg-public-bg text-public-text">
    <PublicSiteHeader onLogin={() => openLogin()} /><main>
      ...
      <div className="mx-auto max-w-[1120px] px-4 md:px-8">{error && <div className="mt-6 flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><span>{error}</span><button type="button" onClick={() => { setError(''); void loadHome(); }} className="font-bold">重试</button></div>}{home?.data_freshness && <p className="mt-5 flex flex-wrap items-center justify-center gap-2 text-center text-[11px] text-public-muted"><CircleAlert className="h-3.5 w-3.5 text-amber-600" />数据口径：{home.data_freshness.is_demo ? '演示数据' : '生产数据'} · 最近更新时间 {formatDate(home.data_freshness.updated_at)}</p>}
        ...
        <section id="services" className="scroll-mt-24 border-t border-public-border py-14 md:py-20"><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow text-public-brand">Services</p><h2 className="mt-2 text-2xl font-black md:text-3xl">从找厂到协同，一站式接上真实业务</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-public-muted">先告诉 AI 你要找什么，链小易会在下一步结合企业能力和业务信号完成匹配。</p></div><span className="rounded-full bg-public-brand-soft px-3 py-1.5 text-xs font-bold text-public-brand">AI 找工厂</span></div><form data-testid="service-ai-form" onSubmit={submitServiceQuery} className="mb-6 flex flex-col gap-2 rounded-2xl border border-public-brand/20 bg-public-brand-soft/40 p-4 md:flex-row md:p-5"><input ref={serviceInputRef} data-testid="service-ai-input" value={serviceQuery} onChange={event => setServiceQuery(event.target.value)} placeholder="例如：找广东能做精密注塑、支持出口的工厂" className="h-12 min-w-0 flex-1 rounded-xl border border-public-border bg-white px-4 text-sm outline-none transition focus:border-public-brand focus:ring-2 focus:ring-public-brand-soft" /><button type="submit" disabled={!serviceQuery.trim()} className="btn-public-primary h-12 justify-center disabled:cursor-not-allowed disabled:opacity-50"><Sparkles className="h-4 w-4" />开始 AI 找工厂 <ArrowRight className="h-4 w-4" /></button></form><div className="grid gap-4 sm:grid-cols-2">{PUBLIC_SERVICES.map(({ key, eyebrow, title, summary, action, icon: Icon }) => <button key={key} type="button" onClick={() => openService(key)} className="group rounded-2xl border border-public-border bg-public-surface p-5 text-left shadow-public transition hover:-translate-y-1 hover:border-public-brand/50 hover:shadow-lg md:p-6"><div className="flex items-start justify-between gap-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-public-brand-soft text-public-brand"><Icon className="h-5 w-5" /></span><ArrowRight className="mt-1 h-4 w-4 text-public-muted transition group-hover:translate-x-1 group-hover:text-public-brand" /></div><p className="mt-5 text-[10px] font-bold uppercase tracking-[0.16em] text-public-brand">{eyebrow}</p><h3 className="mt-2 text-lg font-black">{title}</h3><p className="mt-2 min-h-12 text-sm leading-6 text-public-muted">{summary}</p><span className="mt-5 inline-flex items-center rounded-lg border border-public-border px-3 py-2 text-xs font-bold text-public-text transition group-hover:border-public-brand group-hover:text-public-brand">{action}</span></button>)}</div></section>
