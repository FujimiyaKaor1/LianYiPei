...
class IndustryNewsArticle(db.Model):
    ...
    provider_article_id = db.Column(db.String(255), unique=True)
    industry_tags = db.Column(db.JSON, default=list)
    chain_stage = db.Column(db.String(40))
    related_enterprise_ids = db.Column(db.JSON, default=list)
    related_product_ids = db.Column(db.JSON, default=list)
    relevance_score = db.Column(db.Integer, default=0, nullable=False)
    relevance_status = db.Column(db.String(20), default="pending", nullable=False)
